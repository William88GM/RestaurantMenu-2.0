...
export default function Dynamic1() {
	...
	const { editionMode, setEditionMode, ediciones, setEdiciones, dragActive, setDragActive, loading, setLoading, showPassword, setShowPassword } =
		useStatesIniciales();
	//Hooks Iniciales Estandar
	const { data, setData, logged, toEliminate, setToEliminate, showMenu, setShowMenu, handleMenu, handleChangeView, viewerMode } = useHooksIniciales({
		isProductPage,
		scroll,
		galleryRef,
		ediciones,
		longitudItemsPrevios,
		namesUrlCategories,
		seUsanImagenes,
		dataEditableRef,
		setEdiciones,
	...
	useEffect(() => {
		console.log("Cambia viewer mode desde Dynamic1: ", viewerMode);
	}, [viewerMode]);
	...
	return (
		<main
			style={{
				backgroundImage: `url(${process.env.NEXT_PUBLIC_URL}/images/background.webp)`,
				backgroundColor: process.env.NEXT_PUBLIC_BACKGROUNDCOLORMAIN,
			}}
		>
			<Header navigateTo={`/`} isHome={isHome} handleMenu={handleMenu} showMenu={showMenu} logged={logged} editionMode={editionMode} />
