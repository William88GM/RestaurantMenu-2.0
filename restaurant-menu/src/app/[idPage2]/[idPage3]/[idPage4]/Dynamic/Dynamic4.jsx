...
export default function Dynamic4({ params }) {
    ...
    //MODO DE VISTA
    const { viewerMode, handleChangeView } = useChangeView(galleryRef)

    //DETALLES
    const { details, handleDetails, p } = useDetalles({ name, name1, name2 })
    ...
    return (
        <main style={{ backgroundImage: `url(${process.env.NEXT_PUBLIC_URL}/images/Flor.webp)` }}>
            ...
            <Section isHome={isHome} namesUrlCategories={namesUrlCategories} previousPage={previousPage} actualPage={actualPage} editionMode={editionMode} viewerMode={viewerMode} handleChangeView={handleChangeView} />

            <Article handleVisionItem={handleVisionItem} namesUrlCategories={namesUrlCategories} dataEditableRef={dataEditableRef} imagesHaveChanged={imagesHaveChanged} dragActive={dragActive} logged={logged} data={data} ediciones={ediciones} EditionMode={EditionMode4} setEdiciones={setEdiciones} NormalMode={NormalMode4} editionModeState={editionMode} galleryRef={galleryRef} cardRef={cardRef} p={p} handleDetails={handleDetails} viewerMode={viewerMode} details={details} viewerModeState={viewerMode} />
